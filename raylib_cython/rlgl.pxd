...
cdef extern from "./libs/rlgl.h":
    ...
    cdef bool rlEnableVertexArray(unsigned int vaoId)     # Enable vertex array (VAO, if supported)
    ...
    cdef bool rlIsStereoRenderEnabled()               # Check if stereo render is enabled
    ...
    cdef bool rlCheckRenderBatchLimit(int vCount)                             # Check internal buffer overflow for a given number of vertex
    ...
    cdef unsigned int rlLoadVertexBuffer(const void *buffer, int size, bool dynamic)            # Load a vertex buffer attribute
    cdef unsigned int rlLoadVertexBufferElement(const void *buffer, int size, bool dynamic)     # Load a new attributes element buffer
    ...
    cdef void rlSetVertexAttribute(unsigned int index, int compSize, int type, bool normalized, int stride, const void *pointer)
    ...
    cdef unsigned int rlLoadTextureDepth(int width, int height, bool useRenderBuffer)               # Load depth texture/renderbuffer (to be attached to fbo)
    ...
    cdef bool rlFramebufferComplete(unsigned int id)                        # Verify framebuffer is complete
    ...
    # Buffer management
    cdef void rlBindImageTexture(unsigned int id, unsigned int index, int format, bool readonly)  # Bind image texture
