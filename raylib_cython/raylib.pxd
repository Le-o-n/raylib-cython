...
cdef extern from "libs/raylib.h":
    ...
    cdef void SetWindowPosition(int x, int y)                       # Set window position on screen (only PLATFORM_DESKTOP)
    cdef void SetWindowMonitor(int monitor)                         # Set monitor for the current window
    cdef void SetWindowMinSize(int width, int height)               # Set window minimum dimensions (for FLAG_WINDOW_RESIZABLE)
    ...
    cdef void UnloadVrStereoConfig(VrStereoConfig config)           # Unload VR stereo config
    ...
    cdef void SetShaderValueV(Shader shader, int locIndex, const void *value, int uniformType, int count)   # Set shader uniform value vector
    cdef void SetShaderValueMatrix(Shader shader, int locIndex, Matrix mat)         # Set shader uniform value (matrix 4x4)
    cdef void SetShaderValueTexture(Shader shader, int locIndex, Texture2D texture) # Set shader uniform value for texture (sampler2d)
    cdef void UnloadShader(Shader shader)                                    # Unload shader from GPU memory (VRAM)

    # Screen-space-related functions
    cdef Ray GetMouseRay(Vector2 mousePosition, Camera camera)      # Get a ray trace from mouse position
    cdef Matrix GetCameraMatrix(Camera camera)                      # Get camera transform matrix (view matrix)
    cdef Matrix GetCameraMatrix2D(Camera2D camera)                  # Get camera 2d transform matrix
    cdef Vector2 GetWorldToScreen(Vector3 position, Camera camera)  # Get the screen space position for a 3d world space position
    cdef Vector2 GetScreenToWorld2D(Vector2 position, Camera2D camera) # Get the world space position for a 2d camera screen space position
    cdef Vector2 GetWorldToScreenEx(Vector3 position, Camera camera, int width, int height) # Get size position for a 3d world space position
    cdef Vector2 GetWorldToScreen2D(Vector2 position, Camera2D camera) # Get the screen space position for a 2d camera world space position
    ...
    cdef void SetRandomSeed(unsigned int seed)                      # Set the seed for the random number generator
    cdef int GetRandomValue(int min, int max)                       # Get a random value between min and max (both included)
    cdef int *LoadRandomSequence(unsigned int count, int min, int max) # Load random values sequence, no values repeated
    cdef void UnloadRandomSequence(int *sequence)                   # Unload random values sequence

    # Misc. functions
    cdef void TakeScreenshot(const char *fileName)                  # Takes a screenshot of current screen (filename extension defines format)
    cdef void SetConfigFlags(unsigned int flags)                    # Setup init configuration flags (view FLAGS)
    cdef void OpenURL(const char *url)                              # Open URL with default system browser (if available)

    # NOTE: Following functions implemented in module [utils]
    #------------------------------------------------------------------
    cdef void TraceLog(int logLevel, const char *text, ...)         # Show trace log messages (LOG_DEBUG, LOG_INFO, LOG_WARNING, LOG_ERROR...)
    cdef void SetTraceLogLevel(int logLevel)                        # Set the current threshold (minimum) log level
    cdef void *MemAlloc(unsigned int size)                          # Internal memory allocator
    cdef void *MemRealloc(void *ptr, unsigned int size)             # Internal memory reallocator
    cdef void MemFree(void *ptr)                                    # Internal memory free

    # Set custom callbacks
    # WARNING: Callbacks setup is intended for advance users
    cdef void SetTraceLogCallback(TraceLogCallback callback)         # Set custom trace log
    cdef void SetLoadFileDataCallback(LoadFileDataCallback callback) # Set custom file binary data loader
    cdef void SetSaveFileDataCallback(SaveFileDataCallback callback) # Set custom file binary data saver
    cdef void SetLoadFileTextCallback(LoadFileTextCallback callback) # Set custom file text data loader
    cdef void SetSaveFileTextCallback(SaveFileTextCallback callback) # Set custom file text data saver
    ...
    cdef unsigned char *CompressData(const unsigned char *data, int dataSize, int *compDataSize)        # Compress data (DEFLATE algorithm), memory must be MemFree()
    cdef unsigned char *DecompressData(const unsigned char *compData, int compDataSize, int *dataSize)  # Decompress data (DEFLATE algorithm), memory must be MemFree()
    cdef char *EncodeDataBase64(const unsigned char *data, int dataSize, int *outputSize)               # Encode data to Base64 string, memory must be MemFree()
    cdef unsigned char *DecodeDataBase64(const unsigned char *data, int *outputSize)                    # Decode Base64 string data, memory must be MemFree()
